...
***********/
        ...
        private void buttonFormAreaLoc_Click(object sender, RoutedEventArgs e)
        {
           ...
           // }
            ...
            MyIP.Children.Clear();
            for (int m = 0; m < AreaResult.Count; m++)//每个区域
            {
                ...
                for (int strokeK = 0; strokeK < GVcnt; strokeK++)//每个笔画
                {
                    ...
                    for (int inm = 0; inm < AreaInPoint.Count; inm++)//每组内点
                    {
                        ...
                        st_X = 0;
                        st_Y = 0;
                        int ptttcnt = st.StylusPoints.Count;
                        st_X = st.StylusPoints[ptttcnt / 2].X;
                        st_Y = st.StylusPoints[ptttcnt / 2].Y;
                        ...
                        for (int inn = 0; inn < AreaInPoint[inm].Count; inn++)//每个内点
                        {
                            ...
                            //else if ((st_Y > minY) && (st_X > minX) && (st_Y < maxY) && (st_X < maxX))
                           // {
                           //     kkk++;
                           // }
                            else
                            ...
                            for (int i = 0; i < AreaResult[m].Count; i++)
                            {
                                Stroke st_AoTu = MyIP.Strokes[AreaResult[m][i] + GVcnt];
                                int ptAotucnt = 0;
                                ptAotucnt = st_AoTu.StylusPoints.Count;
                                
                                if ((int)StrokeDirectMax[AreaResult[m][i]] == 1)
                                {
                                    if ((double)StrokeLineH[HangShu] == st_AoTu.GetBounds().Y + st_AoTu.GetBounds().Height * 0.5)
                                    {

                                        if ((st_AoTu.StylusPoints[ptAotucnt / 2].Y - st_AoTu.GetBounds().Y) > st_AoTu.GetBounds().Height * 0.5)
                                        {
                                            minY = (double)(StrokeLineH[HangShu]) + st_AoTu.GetBounds().Height * 0.5;
                                            //StrokeLineH[HangShu] = (double)(StrokeLineH[HangShu]) + st_AoTu.GetBounds().Height * 0.5;
                                        }
                                        else
                                        {
                                            minY = (double)(StrokeLineH[HangShu]) - st_AoTu.GetBounds().Height * 0.5;
                                    ...
                                    if ((double)StrokeLineH[HangShu + 1] == st_AoTu.GetBounds().Y + st_AoTu.GetBounds().Height * 0.5)
                                    {
                                        if ((st_AoTu.StylusPoints[ptAotucnt / 2].Y - st_AoTu.GetBounds().Y) > st_AoTu.GetBounds().Height * 0.5)
                                        {
                                            maxY = (double)(StrokeLineH[HangShu+1]) + st_AoTu.GetBounds().Height * 0.5;
                                        }
                                        else
                                        {
                                            maxY = (double)(StrokeLineH[HangShu+1]) - st_AoTu.GetBounds().Height * 0.5;
                                ...
                                if ((int)StrokeDirectMax[AreaResult[m][i]] == 0)
                                {
                                    if((double)StrokeLineV[LieShu]==st_AoTu.GetBounds().X + st_AoTu.GetBounds().Width * 0.5)
                                    {
                                        if ((st_AoTu.StylusPoints[ptAotucnt / 2].X - st_AoTu.GetBounds().X) > st_AoTu.GetBounds().Width * 0.5)
                                        {
                                            minX = (double)(StrokeLineV[LieShu]) + st_AoTu.GetBounds().Width * 0.5;
                                        }
                                        else
                                        {
                                            minX = (double)(StrokeLineV[LieShu]) - st_AoTu.GetBounds().Width * 0.5;
                                    ...
                                    if ((double)StrokeLineV[LieShu+1] == st_AoTu.GetBounds().X + st_AoTu.GetBounds().Width * 0.5)
                                    {
                                        if ((st_AoTu.StylusPoints[ptAotucnt / 2].X - st_AoTu.GetBounds().X) > st_AoTu.GetBounds().Width * 0.5)
                                        {
                                            maxX = (double)(StrokeLineV[LieShu+1]) + st_AoTu.GetBounds().Width * 0.5;
                                        }
                                        else
                                        {
                                            maxX = (double)(StrokeLineV[LieShu+1]) - st_AoTu.GetBounds().Width * 0.5;
                                ...
                                if ((st_Y > minY) && (st_X > minX) && (st_Y < maxY) && (st_X < maxX))
                                {
                                   kkk++;
                                 }
                            }
